This will walk you through installing HuskSync on your network of Spigot servers. Please check your server's [[Compatibility]] and download the correct version of HuskSync for your server.
...
- Navigate to the new config file on each server (`~/plugins/HuskSync/config.yml` on Spigot)
...
- Navigate to the server name file on each server (`~/plugins/HuskSync/server.yml` on Spigot)
